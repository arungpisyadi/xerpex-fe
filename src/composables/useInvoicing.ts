...
export function useInvoicing() {
  ...
  const draftQuotes = computed(() => quotes.value.filter((quote) => quote.status === 'draft'))

  const sentQuotes = computed(() => quotes.value.filter((quote) => quote.status === 'sent'))

  const acceptedQuotes = computed(() => quotes.value.filter((quote) => quote.status === 'accepted'))
  ...
  const completeQuoteToPaymentFlow = async (
    customerData: any,
    quoteData: any,
    paymentData: any,
  ) => {
