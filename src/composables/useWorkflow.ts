...
export function useQuoteWorkflow() {
  ...
  const removePackage = (packageId: number) => {
    const index = workflowData.selectedPackages.findIndex((item) => item.package.id === packageId)
    if (index !== -1) {
  ...
  const updatePackageQuantity = (packageId: number, quantity: number) => {
    const item = workflowData.selectedPackages.find((item) => item.package.id === packageId)
    if (item) {
