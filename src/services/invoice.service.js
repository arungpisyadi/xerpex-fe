  ...
  async downloadInvoice(id, filename) {
    try {
      ...
      if (!validation.isValid) {
        ...
        if (validation.details) {
          console.log('[PDF-DEBUG] Corruption details:', {
            ...
            first20AsHex: validation.details.first20Bytes?.map(b => '0x' + b.toString(16).padStart(2, '0')).join(' ')
          });
        }
      ...
      if (error.message?.includes('timeout')) {
        ...
        userMessage = 'The PDF file appears to be corrupted. Please contact support with invoice ID: ' + id;
