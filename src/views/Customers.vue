<template>
  <admin-layout>
    <div class="mb-6">
      <page-breadcrumb page-title="Customers" />
    </div>
    ...
    >
      <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h2 class="text-title-md2 font-bold text-black dark:text-white">Customer Management</h2>
    ...
    >
      ...
      >
        <div class="mb-5 flex items-center justify-between">
          <h4 class="text-xl font-semibold text-black dark:text-white">Customer Details</h4>
          ...
          >
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                >
                <p class="text-black dark:text-white">
                  {{ selectedCustomer.billing_address || 'Same as address' }}
                </p>
              </div>
            </div>
          </div>
        ...
        <div class="mt-6 flex justify-end gap-4">
          ...
          <button
            v-if="getCustomerStatus(selectedCustomer.status) && canDelete"
    ...
    >
      ...
      >
        ...
        <FormKit type="form" :actions="false" @submit="submitCustomer">
    ...
    >
      ...
      >
        <div class="mb-5 flex items-center justify-between">
          <h4 class="text-xl font-semibold text-black dark:text-white">Confirm Deactivation</h4>
        ...
        <div class="mb-6">
          <p class="text-gray-600 dark:text-gray-400">
            Are you sure you want to deactivate this customer? This action will make the customer
            inactive but can be reversed later.
          </p>
        </div>
...
export default {
  ...
  setup() {
    const { customers, loading, error, fetchCustomers, createCustomer, activateCustomer } =
      useInvoicing()
  ...
  computed: {
    canCreate() {
      return this.permissions.canPerform(SystemModule.CUSTOMERS, PermissionAction.CREATE)
    },
    canUpdate() {
      return this.permissions.canPerform(SystemModule.CUSTOMERS, PermissionAction.UPDATE)
    },
    canDelete() {
      return this.permissions.canPerform(SystemModule.CUSTOMERS, PermissionAction.DELETE)
    },
    filteredCustomers() {
  ...
  methods: {
    ...
    createNewCustomer() {
      if (!this.canCreate) {
        console.error('You do not have permission to create customers')
        return
      }
    ...
    editCustomer(customer) {
      if (!this.canUpdate) {
        console.error('You do not have permission to edit customers')
        return
      }
    ...
    confirmDeactivateCustomer(customer) {
      if (!this.canDelete) {
        console.error('You do not have permission to deactivate customers')
        return
      }
