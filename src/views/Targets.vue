  ...
  <admin-layout>
    ...
    <div class="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      ...
      <template v-else>
        <!-- Sales Users Targets -->
        <div class="space-y-6">
          <div v-for="salesUser in salesUsers" :key="salesUser.id" class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            <!-- Accordion Header -->
            <div
            ...
            >
              <div class="flex items-center">
                <div class="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center mr-3">
                  <span class="text-sm font-medium text-primary">{{ salesUser.full_name?.charAt(0)?.toUpperCase() }}</span>
                </div>
                <div>
                  <h3 class="text-lg font-semibold text-black dark:text-white">{{ salesUser.full_name }}</h3>
                  <p class="text-sm text-gray-500">{{ salesUser.email }}</p>
                </div>
              </div>
              <div class="flex items-center">
                <button
            ...
            <!-- Accordion Content -->
            <div v-if="expandedUsers[salesUser.id]" class="border-t border-stroke dark:border-strokedark">
              <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  <div v-for="month in months" :key="month.value" class="rounded-sm border border-stroke bg-gray-50 p-4 dark:border-strokedark dark:bg-meta-4">
                    <h4 class="text-md font-medium text-black dark:text-white mb-4">{{ month.label }}</h4>
                    <FormKit
                      type="form"
                      :actions="false"
                      @submit="saveTarget(salesUser.id, month.value)"
                    ...
                    >
                      <FormKit
                        ...
                        label="Target Amount"
                        v-model="targets[salesUser.id][month.value]"
                        placeholder="0.00"
                        min="0"
                        step="1000"
                        ...
                        :validation-messages="{
                          ...
                          min: 'Target amount must be greater than 0'
                        }"
                      />
  ...
  methods: {
    ...
    distributeTargets(targets) {
      targets.forEach(target => {
        ...
        if (this.targets[target.user_id]) {
          this.targets[target.user_id][target.month] = target.target_amount || '';
        }
      });
    },
    ...
    async saveTarget(userId, month) {
      try {
        ...
        const existingTargets = await TargetsService.getAllTargets();
        const existingTarget = existingTargets.find(t => t.user_id === userId && t.month === month);
