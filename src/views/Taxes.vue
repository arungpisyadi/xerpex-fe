  ...
  <admin-layout>
    ...
    >
      <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h2 class="text-title-md2 font-bold text-black dark:text-white">Tax Management</h2>
    ...
    <!-- Tax Calculator -->
    <div
      class="mt-6 bg-white px-2 pt-2 pb-0.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-4 xl:pb-0"
    >
      ...
      <div v-if="calculator.result" class="mt-4 p-4 bg-gray-100 dark:bg-gray-800 rounded">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div>
            ...
            <span class="ml-2 font-medium text-black dark:text-white"
              >IDR {{ calculator.result.subtotal.toFixed(2) }}</span
            >
          </div>
          <div>
            <span class="text-gray-500 dark:text-gray-400">Tax Total:</span>
            <span class="ml-2 font-medium text-black dark:text-white"
              >IDR {{ calculator.result.tax_total.toFixed(2) }}</span
            >
          </div>
          <div>
            <span class="text-gray-500 dark:text-gray-400">Total:</span>
            <span class="ml-2 font-medium text-black dark:text-white"
              >IDR {{ calculator.result.total.toFixed(2) }}</span
            >
          </div>
        </div>
        <div v-if="calculator.result.tax_breakdown.length > 0" class="mt-3">
          ...
          <div class="space-y-1">
            <div
              v-for="breakdown in calculator.result.tax_breakdown"
              :key="breakdown.name"
              class="flex justify-between text-sm"
            >
              <span class="text-gray-500 dark:text-gray-400"
                >{{ breakdown.name }} ({{ breakdown.percentage }}%):</span
              >
              <span class="font-medium text-black dark:text-white"
                >IDR {{ breakdown.amount.toFixed(2) }}</span
              >
            </div>
          </div>
        </div>
