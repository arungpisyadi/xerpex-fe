  ...
  <admin-layout>
    ...
    >
      <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h2 class="text-title-md2 font-bold text-black dark:text-white">Users Management</h2>
    ...
    >
      ...
      >
        ...
        <FormKit type="form" :actions="false" @submit="saveUser">
          <FormKit
            type="text"
            name="username"
            label="Username"
    ...
    >
      ...
      >
        <div class="mb-5 flex items-center justify-between">
          <h4 class="text-xl font-semibold text-black dark:text-white">User Details</h4>
              ...
              <div class="space-y-3">
                ...
                <div v-if="selectedUser.is_active !== undefined">
                  ...
                  <span
                    :class="[
                      'inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ml-2',
                      selectedUser.is_active ? 'bg-success text-success' : 'bg-red-500 text-danger',
                    ]"
                  >
  ...
  methods: {
    ...
    openAddUserModal() {
      ...
      if (!this.canCreate) {
        ...
        this.showNotification(
          'error',
          'You do not have permission to create users. Please contact your administrator if you believe this is an error.',
        )
        return
      }
    ...
    async editUser(user) {
      ...
      if (!this.canUpdate) {
        ...
        this.showNotification(
          'error',
          'You do not have permission to edit users. Please contact your administrator if you believe this is an error.',
        )
        return
      }
      ...
      try {
        ...
        this.showNotification(
          'error',
          'An unexpected error occurred while preparing the edit form. Please try again.',
        )
      }
    },
    confirmDeleteUser(user) {
      ...
      if (!this.canDelete) {
        ...
        this.showNotification(
          'error',
          'You do not have permission to delete users. Please contact your administrator if you believe this is an error.',
        )
        return
      }
      ...
      switch (fieldName) {
        ...
        case 'role':
          if (!this.userForm.role) {
            ...
            !['admin', 'manager', 'finance', 'sales', 'survey', 'staff'].includes(
              this.userForm.role,
            )
          ) {
    ...
    async saveUser() {
      ...
      if (!this.validateForm()) {
        ...
        this.showNotification(
          'error',
          `Please fix the ${errorCount} validation error${errorCount > 1 ? 's' : ''} before submitting`,
        )
        return
      }
    ...
    async deleteUser() {
      ...
      try {
        ...
        if (error.response?.status === 400) {
          errorMessage =
            error.response.data?.message || 'Cannot delete this user due to data constraints'
        } else if (error.response?.status === 401) {
          errorMessage = 'Authentication failed. Please log in again.'
        } else if (error.response?.status === 403) {
