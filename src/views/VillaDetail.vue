<template>
  <admin-layout>
    <div class="mb-6">
      <page-breadcrumb page-title="Villa Detail" />
    </div>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
          ...
          >
            <div class="flex flex-col items-center">
              ...
              <div class="flex items-center mb-2">
                <span
                  v-for="i in Math.floor(villa.rating || 0)"
                  :key="`star-${i}`"
              ...
              <div class="w-full border-t border-stroke pt-4 dark:border-strokedark">
                <div class="mb-3 flex items-center">
                  ...
                  <span class="text-sm text-black dark:text-white"
                    >Capacity: {{ villa.capacity }} persons</span
                  >
                </div>
                <div class="mb-3 flex items-center">
                  <span class="mr-2 text-gray-500">
                ...
                <div class="mb-3 flex items-center">
                  ...
                  <span class="text-sm text-black dark:text-white"
                    >Bathrooms: {{ villa.bathrooms }}</span
                  >
                </div>
        ...
        <div class="md:col-span-2">
          ...
          >
            <h4 class="mb-4 text-xl font-semibold text-black dark:text-white">Description</h4>
          ...
          <div
            class="rounded-sm border border-stroke bg-white p-6 shadow-default dark:border-strokedark dark:bg-boxdark"
          >
            <h4 class="mb-4 text-xl font-semibold text-black dark:text-white">Amenities</h4>
            <div class="grid grid-cols-2 gap-4">
              <div
                v-for="(amenity, index) in villa.amenities"
                :key="index"
      ...
      <div class="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div class="pt-4 border-t border-stroke dark:border-strokedark">
            <div class="flex justify-between">
              ...
              <span class="text-base font-medium text-black dark:text-white"
                >{{ villa.minimum_stay || 1 }} night(s)</span
              >
            </div>
          </div>
        </div>
        ...
        <div
          class="rounded-sm border border-stroke bg-white p-6 shadow-default dark:border-strokedark dark:bg-boxdark"
        >
          <h4 class="mb-4 text-xl font-semibold text-black dark:text-white">Availability</h4>
          <div class="mb-4">
            ...
            <div v-else>
              <div
                ...
                :key="index"
                class="mb-3 pb-3 border-b border-stroke dark:border-strokedark last:mb-0 last:pb-0 last:border-0"
              >
                <div class="flex justify-between mb-1">
                  <span class="text-sm font-medium text-black dark:text-white"
                    >{{ formatDate(booking.check_in) }} - {{ formatDate(booking.check_out) }}</span
                  >
                  <span class="text-xs px-2 py-1 rounded" :class="getStatusClass(booking.status)">{{
                    booking.status
                  }}</span>
                </div>
                <div class="text-xs text-gray-500">
                  {{ getDuration(booking.check_in, booking.check_out) }} nights
                </div>
              </div>
            </div>
          </div>
          <div class="pt-4 border-t border-stroke dark:border-strokedark">
            <button
              class="flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium text-white rounded-lg bg-brand-500 hover:bg-brand-600 w-full"
            >
              Check Availability
            </button>
          </div>
